...
// ChooseEwoqKeyOrLedger provides a mock function with given fields: askForEwoq, goal
func (_m *Prompter) ChooseEwoqKeyOrLedger(askForEwoq bool, goal string) (prompts.KeySource, error) {
	ret := _m.Called(askForEwoq, goal)

	var r0 prompts.KeySource
	var r1 error
	if rf, ok := ret.Get(0).(func(bool, string) (prompts.KeySource, error)); ok {
		return rf(askForEwoq, goal)
	}
	if rf, ok := ret.Get(0).(func(bool, string) prompts.KeySource); ok {
		r0 = rf(askForEwoq, goal)
	} else {
		r0 = ret.Get(0).(prompts.KeySource)
	}

	if rf, ok := ret.Get(1).(func(bool, string) error); ok {
		r1 = rf(askForEwoq, goal)
	} else {
		r1 = ret.Error(1)
	}
